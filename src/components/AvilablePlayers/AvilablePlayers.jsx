...
const AvilablePlayers = ({ PlayersPromise,setAvilableBalence,AvilableBalence }) => {
  ...
  return (
    <div className="bg-white text-black min-h-screen mb-5 ">
      <div className=" mx-auto max-w-[1200px] grid grid-cols-1 md:grid-cols-3 gap-4">
        {PlayerData.map(player => <PlayerCard AvilableBalence={AvilableBalence} setAvilableBalence={setAvilableBalence} player={player}></PlayerCard> )}
      </div>
    </div>
  );
};
